...
from caragent_agent.third_party.from_langgraph.tool_node import ToolNode
...
def create_react_agent(
    ...
    *,
    ...
    logger: Optional[Callable[[str], None]] = None, # Add logger
    **deprecated_kwargs: Any,
    ...
    if isinstance(tools, ToolNode):
        ...
        tool_node = tools
        if logger and hasattr(tool_node, 'logger') and tool_node.logger is None:
             tool_node.logger = logger
    else:
        llm_builtin_tools = [t for t in tools if isinstance(t, dict)]
        tool_node = ToolNode([t for t in tools if not isinstance(t, dict)], logger=logger)
    ...
    if not is_dynamic_model:
        ...
        if (
            ...
            # MODIFIED: Add parallel_tool_calls=False to prevent parallel tool execution
            # This ensures the model generates only one tool call at a time
    ...
    def call_model(
        ...
        # Print LLM response
        if logger:
             logger(f"React Agent: LLM Response: {response.content}")
        # print(f"React: LLM Response: {response.content}")
