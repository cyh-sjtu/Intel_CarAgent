...
class ToolNode(RunnableCallable):
    ...
    def _run_one(
        ...
        try:
            if self.logger:
                self.logger(f"Invoking tool {call['name']} with args {call['args']}")
            # print(f"Invoking tool {call['name']} with args {call['args']}")
            call_args = {**call, **{"type": "tool_call"}}
            response = self.tools_by_name[call["name"]].invoke(call_args, config)
            response_str = str(response)
            if self.logger:
                self.logger(f"Tool {call['name']} returned {response_str[:100]}..." if len(response_str) > 100 else f"Tool {call['name']} returned {response_str}")
            # print(f"Tool {call['name']} returned {response_str[:100]}..." if len(response_str) > 100 else f"Tool {call['name']} returned {response_str}")
