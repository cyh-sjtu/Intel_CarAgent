...
class AttachedImageKeyframeMatcherTool(ToolBase):
    ...
    def _rerank_direct_hybrid_candidates(
        ...
        *,
        ...
        if str(focus or "").strip().lower() == "object":
            question = (
                "You are evaluating object-centric staging keyframes for a mobile robot. "
                "The stitched image contains one QUERY photo and candidate keyframes labeled by KF id. "
                "Select the candidate that is most useful for navigating near the target object shown or "
                "described by the QUERY image. Highest priority: the target object itself should be visible "
                "in the candidate keyframe and should be clear, complete, close enough, not heavily occluded, "
                "and suitable for later live robot-scene object localization. Use the surrounding scene layout "
                "only to disambiguate the correct place among similar candidates. If a candidate has strong "
                "same-place spatial layout but does not show the target object, rank it below a candidate "
                "where the target object is clearly visible and localizable. Do not select a candidate merely "
                "because it contains a similar generic object; prefer the one that matches both the target "
                "object and its surrounding place context. Camera viewpoint, crop, tilt, and lighting can differ. "
                f"Focus: object. User query if any: {query_text}\n"
                "Return only JSON inside <answer>: "
                "{\"ranked_keyframe_ids\":[id1,id2,...],\"best_keyframe_id\":id1,"
                "\"reason\":\"short evidence focused on target-object visibility and localization usefulness\"}"
            )
        else:
            question = (
                ...
                f"Focus: scene. User query if any: {query_text}\n"
                "Return only JSON inside <answer>: "
                "{\"ranked_keyframe_ids\":[id1,id2,...],\"best_keyframe_id\":id1,"
