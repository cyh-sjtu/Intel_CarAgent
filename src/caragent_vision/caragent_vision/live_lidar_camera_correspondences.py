...
class LiveLidarCameraCorrespondences(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            "Controls: click image point, h/j/k/l move image cursor, p use cursor, i type pixel, "
    ...
    def _on_mouse(self, event, x, y, flags, userdata) -> None:
        if self.latest_image is not None and y < self.latest_image.shape[0]:
            self.hover_pixel = (int(x), int(y))
            self.cursor_pixel = (int(x), int(y))
        if event == cv2.EVENT_LBUTTONDOWN:
            if self.latest_image is not None and y >= self.latest_image.shape[0]:
                self.get_logger().info("Click ignored: please click the upper image panel, not the scan panel.")
                return
    ...
    def _draw(self) -> None:
        ...
        if self.clicked is not None:
            cv2.drawMarker(image, self.clicked, (0, 255, 255), markerType=cv2.MARKER_CROSS, markerSize=18, thickness=2)
        cv2.putText(image, "h/j/k/l move | p=use cursor i=type | s=save q=quit", (10, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 1)
        cv2.putText(image, f"saved={self.saved_count}", (10, 48), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 1)
        coord_text = f"cursor={self.cursor_pixel}"
        if self.clicked is not None:
            coord_text += f" selected={self.clicked}"
        cv2.putText(image, coord_text, (10, 72), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 1)

        if scan is None:
            panel = np.full((h, w, 3), 230, dtype=np.uint8)
            cv2.putText(panel, "waiting for /scan", (20, h // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (20, 20, 20), 2)
        else:
            panel = self._draw_scan_panel(scan, w, h)
